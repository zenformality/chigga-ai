...
export default function CharacterCard({ character, onSelect, isSelected, isOwner, onEdit }: CharacterCardProps) {
  return (
    <motion.div
      whileHover={{ y: -8 }}
      transition={{ duration: 0.5, ease: [0.19, 1, 0.22, 1] }}
      className={cn(
        "relative rounded-none text-left transition-all duration-700 group overflow-hidden h-[420px] flex flex-col",
    ...
    >
      <button
        ...
        className="absolute inset-0 z-10 p-6 flex flex-col text-left"
      >
      ...
      {isOwner && onEdit && (
        <button
          ...
          className="absolute top-4 left-4 z-30 p-2 bg-black/60 backdrop-blur-md rounded-full border border-white/20 hover:border-gold-500/50 hover:text-gold-500 text-white/60 transition-all opacity-0 group-hover:opacity-100"
